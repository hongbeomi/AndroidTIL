...
안드로이드의 시스템 이벤트나 기타 앱에서 특정 이벤트가 발생했을 때 이를 수신는 컴포넌트입니다. 앱에서 특정 브로드캐스트를 수신하는 리시버를 등록하면 시스템은 해당 브로드캐스트가 발생할 때마다 해당 앱에 브로드캐스트를 자동으로 라우팅해주는데, 이 동작은 옵저버 패턴으로 구현되어 있습니다.
